...
const AddVolunteer = ({ onBack, onSave }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await onSave(newVolunteerData);
      setSnackbar({ open: true, message: 'Thêm tình nguyện viên thành công!', severity: 'success' });
      setFormData({ name: '', email: '', phone: '', password: '' });
      setAvatar(null);
      setAvatarPreview(null);
    } catch (error) {
      setSnackbar({ open: true, message: 'Có lỗi xảy ra khi thêm tình nguyện viên', severity: 'error' });
    } finally {
      setLoading(false);
    }
  };
    ...
    <Box sx={{ pb: 4, background: 'linear-gradient(135deg, #f8f9fa 0%, #e3f2fd 100%)', minHeight: '100vh' }}>
      ...
      <Box sx={{ mb: 4, p: 3, background: 'linear-gradient(135deg, #1a237e 0%, #3949ab 100%)', borderRadius: 3, color: 'white' }}>
        ...
        <Typography variant="body1" sx={{ opacity: 0.9 }}>
          Điền thông tin để thêm tình nguyện viên mới vào hệ thống
        </Typography>
      </Box>
          ...
          <form onSubmit={handleSubmit}>
            <Box display="flex" flexDirection="column" gap={3}>
              ...
              <Box sx={{ display: 'flex', gap: 2, justifyContent: 'flex-end' }}>
                ...
                >
                  {loading ? 'Đang lưu...' : 'Lưu tình nguyện viên'}
                </Button>
              </Box>
            </Box>
          </form>
