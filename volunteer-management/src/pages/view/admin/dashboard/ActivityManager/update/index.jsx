...
import {
  ...
  IconButton, Alert, Snackbar, CircularProgress, Divider, Avatar, Grid,
...
const UpdateActivity = ({ activity, onClose, onUpdate }) => {
  const [formData, setFormData] = useState({
    ...
    tag_ids: activity?.tags?.map(tag => tag.id) || []
  });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const updateData = {
        ...
        participants_current: parseInt(formData.participants_current) || 0,
        participants_max: parseInt(formData.participants_max),
        participants_percentage: parseInt(formData.participants_percentage) || 0,
      ...
      await onUpdate(activity.id, updateData);
      setSnackbar({ open: true, message: 'Cap nhat hoat dong thanh cong!', severity: 'success' });
    } catch (error) {
      setSnackbar({ open: true, message: 'Co loi xay ra khi cap nhat hoat dong', severity: 'error' });
    } finally {
      setLoading(false);
    }
  };
    ...
    <Box sx={{ pb: 4, background: 'linear-gradient(135deg, #f8f9fa 0%, #e3f2fd 100%)', minHeight: '100vh' }}>
      ...
      <Box sx={{ mb: 4, p: 3, background: 'linear-gradient(135deg, #1a237e 0%, #3949ab 100%)', borderRadius: 3, color: 'white' }}>
        ...
        <Typography variant="body1" sx={{ opacity: 0.9 }}>
          Cập nhật thông tin hoạt động: <strong>{activity.title}</strong>
        </Typography>
      </Box>
          ...
          <form onSubmit={handleSubmit}>
            <Box display="flex" flexDirection="column" gap={3}>
                ...
                <Box sx={{ 
                  ...
                  <Typography variant="subtitle2" sx={{ 
                    ...
                    Thong tin he thong:
                     ...
                     <Box>
                       <Typography variant="body2" color="text.secondary">
                         ID: <strong>#{activity.id}</strong>
                       </Typography>
                     </Box>
                     <Box>
                       <Typography variant="body2" color="text.secondary">
                         Ngày tạo: <strong>{formatDateTime(activity.created_at)}</strong>
                       </Typography>
                     ...
                     <Box>
                       <Typography variant="body2" color="text.secondary">
                         Cập nhật lần cuối: <strong>{formatDateTime(activity.updated_at)}</strong>
                       </Typography>
                       <Typography variant="caption" color="text.secondary" sx={{ display: 'block' }}>
                     ...
                     <Box>
                       <Typography variant="body2" color="text.secondary">
                         Người tham gia hiện tại: <strong>{activity.participants_current || 0}</strong>
                       </Typography>
                     </Box>
                     <Box>
                       <Typography variant="body2" color="text.secondary">
                         Số timeline items: <strong>{formData.timeline.length}</strong>
                       </Typography>
                     </Box>
                     <Box>
                       <Typography variant="body2" color="text.secondary">
                         Có hình ảnh: <strong>{formData.image ? 'Có' : 'Không'}</strong>
                       </Typography>
                     </Box>
                   </Box>
                </Box>
              ...
              <Box sx={{ 
                ...
                >
                  {loading ? 'Dang cap nhat...' : 'Cap nhat hoat dong'}
                </Button>
              </Box>
            </Box>
          </form>
