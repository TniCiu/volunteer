...
const AddTag = ({ onBack, onSave }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setSnackbar({ open: true, message: 'Có lỗi xảy ra khi thêm loại hoạt động', severity: 'error' });
    } finally {
      setLoading(false);
    }
  };
    ...
    <Box sx={{ pb: 4, background: 'linear-gradient(135deg, #f8f9fa 0%, #e3f2fd 100%)', minHeight: '100vh' }}>
      ...
      <Box sx={{ mb: 4, p: 3, background: 'linear-gradient(135deg, #1a237e 0%, #3949ab 100%)', borderRadius: 3, color: 'white' }}>
        ...
        <Typography variant="body1" sx={{ opacity: 0.9 }}>
          Điền thông tin để thêm loại hoạt động mới vào hệ thống
        </Typography>
      </Box>
          ...
          <form onSubmit={handleSubmit}>
            <Box display="flex" flexDirection="column" gap={3}>
              ...
              <Box sx={{ 
                ...
                >
                  {loading ? 'Đang lưu...' : 'Lưu loại hoạt động'}
                </Button>
              </Box>
            </Box>
          </form>
